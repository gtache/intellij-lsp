/* Adapted from lsp4e */
package com.github.gtache.lsp.languageserver.wrapper
...
import com.github.gtache.lsp.languageserver.requestmanager.SimpleRequestManager
import com.github.gtache.lsp.languageserver.status.ServerStatus
import com.github.gtache.lsp.languageserver.status.ServerStatusWidget
import com.github.gtache.lsp.languageserver.status.ServerStatusWidgetFactory
...
class LanguageServerWrapperImpl(
    override val serverDefinition: Definition,
    ...
    private fun getAllPotentialEditors(): List<Editor> {
        return FileUtils.getAllOpenedEditors(project).filter { e -> serverDefinition.extensions.contains(FileUtils.editorToExtension(e)) }
    }
            ...
            if (rootPath != null) {
                try {
                    ...
                    client?.let { c ->
                        ...
                        val launcher =
                            if (project.service<LSPPersistentProjectSettings>().projectState.idToSettings[serverDefinition.id]?.isLogging == true) LSPLauncher.createClientLauncher(
                                c,
                                inputStream,
                                outputStream,
                                false,
    ...
    override fun getLanguageId(contentTypes: Array<String>): String? {
        return if (contentTypes.any { serverDefinition.extensions.contains(it) }) serverDefinition.id else null
    }
    ...
    private fun loadConfiguration(): Unit {
        ...
        configuration = service<ConfigurationProvider>().getConfiguration(file)
    }
    ...
    override fun didChangeWatchedFiles(uri: String, type: FileChangeType): Unit {
        ...
        try {
            if (uriFile.exists() && confFile.exists() && Files.isSameFile(uriFile.toPath(), confFile.toPath())) {
                configuration = service<ConfigurationProvider>().getConfiguration(confFile)
            }
        } catch (e: Exception) {
            logger.warn(e)
        } catch (t: Throwable) {
