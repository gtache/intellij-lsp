...
object ScalaPsiUtil {
  ...
  def getMethodPresentableText(method: PsiMethod, subst: ScSubstitutor): String = {
    method match {
  ...
  * */
  ...
  def findInstanceBinding(instance: ScExpression): Option[ScBindingPattern] = {
    ...
    if (bindings.size == 1 && expr.exists(c => c == instance)) Option(bindings.head)
    else {
      for (bind <- bindings) {
  ...
  /**
    ...
    */
  def isSAMEnabled(element: PsiElement): Boolean = true
  ...
  object MethodValue {
    ...
    private def isSimpleUnderscore(expr: ScExpression) = expr match {
      ...
      case typed: ScTypedStmt => Option(typed.expr).map(_.getText).exists(c => c == "_")
      case _ => false
    }
