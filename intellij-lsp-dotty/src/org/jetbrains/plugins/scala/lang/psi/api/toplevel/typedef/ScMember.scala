  ...
  private def containingClassInner: ScTemplateDefinition = {
    def isCorrectContext(found: ScTemplateDefinition): Boolean = {
      ...
      context == found.extendsBlock ||
        found.extendsBlock.templateBody.exists(c => c == context) ||
        found.extendsBlock.earlyDefinitions.exists(c => c == context) ||
        found.physicalExtendsBlock.templateBody.exists(c => c == context) // in case a member is not present in the desugared extends block (e.g. deleted by a macro)
    }
