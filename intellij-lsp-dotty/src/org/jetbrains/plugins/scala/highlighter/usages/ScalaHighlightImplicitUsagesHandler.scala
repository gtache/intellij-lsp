...
object ScalaHighlightImplicitUsagesHandler {
  ...
  private implicit class ImplicitTarget(target: PsiElement) {
    ...
    def isTarget(named: PsiElement): Boolean = named match {
      case `target`                                                          => true
      case f: ScFunction if f.getSyntheticNavigationElement.exists(c => c==target) => true
      case _                                                                 => false
    }
    ...
    def forTypeElem(typeElem: ScSimpleTypeElement) = {
      def newTd =
        Option(PsiTreeUtil.getParentOfType(typeElem, classOf[ScNewTemplateDefinition]))
          .filter(_.constructor.flatMap(_.simpleTypeElement).exists(c => c==typeElem))

      def constructor =
        Option(PsiTreeUtil.getParentOfType(typeElem, classOf[ScConstructor]))
          .filter(_.simpleTypeElement.exists(c => c==typeElem))
